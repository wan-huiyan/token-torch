...
check("decodeProjectDir strips worktree suffix and path encoding to base name", () => {
  assert.equal(decodeProjectDir("-Users-<user>-Documents-claude-retrospectives"), "claude-retrospectives");
  assert.equal(
    decodeProjectDir("-Users-<user>-Documents-the-handover-repo--claude-worktrees-gifted-dirac-3b05ca"),
    "the-handover-repo",
  );
  assert.equal(decodeProjectDir("-Users-<user>"), "<user>"); // home catch-all → last segment
});
